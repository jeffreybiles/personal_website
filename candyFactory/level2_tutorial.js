...
var a_bar_walks_into_my_tummy_events = function(){
  ...
  var lightbulb_in = find_incoming(filter_elements(lightbulb('')))
  if (filter_elements(sensor('', 'empty')).length == 3) {
    //the start
  ...
  } else if (has_element_where(sensor('bar', 'active'))  && has_element_where(lightbulb('empty'))) {
